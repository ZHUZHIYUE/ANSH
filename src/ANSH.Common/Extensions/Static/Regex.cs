...
public static class ANSHCommonExtensionsRegex {
    ...
    static Dictionary<ANSHRegexType, string> _RegexList = new Dictionary<ANSHRegexType, string> () {
        [ANSHRegexType.空或正整数] = @"^[1-9]\d*$|(^\s*$)", [ANSHRegexType.文件名字符串] = "^[A-z0-9\u4e00-\u9fa5 ]{0,}$", [ANSHRegexType.正整数] = @"^[1-9]\d*$", [ANSHRegexType.身份证] = "^[a-zA-Z0-9]{15}$|^[a-zA-Z0-9]{18}$", [ANSHRegexType.文件路径] = @"[a-zA-Z]:(\\([0-9a-zA-Z]+))+|(\/([0-9a-zA-Z]+))+", [ANSHRegexType.URL路径] = @"http(s)?://(([0-9]{1,3}\.){3}[0-9]{1,3}(:([0-9]{1,4}))?|([\w-]+\.)+[\w-]+)(/[\w- ./?%&=]*)?", [ANSHRegexType.密码] = "^(?![A-Za-z]+$)(?![A-Z\\d]+$)(?![A-Z\\W]+$)(?![a-z\\d]+$)(?![a-z\\W]+$)(?![\\d\\W]+$)\\S{6,16}$", [ANSHRegexType.手机号] = "^[1][35789][0-9]{9}$"
    };
}
